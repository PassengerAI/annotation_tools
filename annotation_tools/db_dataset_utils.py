    ...
    if len(categories) > 0:
        ...
        try:
            ...
            if len(panic) > 0:
                raise
            print(
                "Attempted to insert duplicate categories, %d new categories inserted" % (
    ...
    if len(images) > 0:
        ...
        try:
            ...
            if len(panic) > 0:
                raise
            print(
                "Attempted to insert duplicate images, %d new images inserted" % (
                    bwe.details['nInserted'],))
    ...
    if len(annotations) > 0:
            ...
            for anno in annotations:
                ...
                if 'keypoints' in anno:
                    for pidx in range(0, len(anno['keypoints']), 3):
                        ...
                        anno['keypoints'][pidx:pidx + 2] = [x / image_width,
                                                            y / image_height]
        ...
        try:
            ...
            if len(panic) > 0:
                raise
            print(
                "Attempted to insert duplicate annotations, %d new annotations inserted" % (
                    bwe.details['nInserted'],))
    ...
    if len(licenses) > 0:
        ...
        try:
            ...
            if len(panic) > 0:
                raise
            print(
                "Attempted to insert duplicate licenses, %d new licenses inserted" % (
                    bwe.details['nInserted'],))
...
def export_dataset(db, denormalize=False,
    ...
    images = list(db.image.find(
        {'id': {'$gte': str(image_id_start),
                '$lte': str(image_id_end)}} if image_id_start else {},
        projection={'_id': False}))
    print("Found %d images" % (len(images),))

    annotations = list(db.annotation.find(
        {'image_id': {'$gte': str(image_id_start),
                      '$lte': str(image_id_end)
                      }} if image_id_start else {},
    ...
    if denormalize:
        ...
        for anno in annotations:
            image_width, image_height = image_id_to_w_h[anno['image_id']]
            x, y, w, h = anno['bbox']
            anno['bbox'] = [x * image_width, y * image_height, w * image_width,
                            h * image_height]
            if 'keypoints' in anno:
                for pidx in range(0, len(anno['keypoints']), 3):
                    x, y = anno['keypoints'][pidx:pidx + 2]
...
def parse_args():
    ...
    parser.add_argument('-a', '--action', choices=['drop', 'load', 'export'],
                        dest='action',
                        help='The action you would like to perform.',
                        required=True)

    parser.add_argument('-d', '--dataset', dest='dataset_path',
                        help='Path to a json dataset file. Used with the `load` action.',
                        type=str,
                        required=False)
    ...
    parser.add_argument('-o', '--output', dest='output_path',
                        help='Save path for the json dataset. Used with the `export` action.',
                        type=str,
                        required=False)
    parser.add_argument('-si', '--start_id', dest='start_id',
                        help='Image start id',
                        type=str,
                        required=False)
    parser.add_argument('-ei', '--end_id', dest='end_id',
                        help='Image end id',
                        type=str,
                        required=False)
...
def main():
    ...
    if action == 'drop':
        ...
        dataset = export_dataset(db, denormalize=args.denormalize,
                                 image_id_start=args.start_id,
                                 image_id_end=args.end_id)
