...
export class LeafletAnnotation extends React.Component {
  ...
  constructor(props) {
    ...
    this.handleKeypointVisibilityChange = this.handleKeypointVisibilityChange.bind(
      this
    );
    ...
    this.checkKeypointAnnotationQueue = this.checkKeypointAnnotationQueue.bind(
      this
    );
    ...
    this.categorySelectionCancelled = this.categorySelectionCancelled.bind(
      this
    );
  }
  ...
  componentDidMount() {
    ...
    let image = L.imageOverlay(this.props.imageElement.src, bounds).addTo(
      leafletMap
    );
    ...
    this.editor = new L.EditToolbar.Edit(leafletMap, {
      featureGroup: this.annotationFeatures
    });
        ...
        if (v > 0) {
          ...
          if (v == 1) {
            marker.setBackground(
              this.createKeypointStripedBackgroundStyle(keypoint_color)
            );
          }
  ...
  annotateKeypoint(keypoint_color, keypoint_name, visibility) {
    ...
    if (visibility == 1) {
      ...
      marker.setBackground(
        this.createKeypointSolidBackgroundStyle(keypoint_color)
      );
    }
  ...
  _drawCreatedEvent(e) {
    ...
    if (this.annotating_keypoint) {
      ...
      // Save off the layer
      this.annotation_layers[this.current_annotationIndex]["keypoints"][
        this.current_keypointIndex
      ] = layer;
      ...
      let keypoint_color =
        category["keypoints_style"][this.current_keypointIndex];
      if (visibility == 1) {
        layer.getElement().style.background = this.createKeypointStripedBackgroundStyle(
          keypoint_color
        );
      } else {
        layer.getElement().style.background = this.createKeypointSolidBackgroundStyle(
          keypoint_color
        );
      }
  ...
  checkKeypointAnnotationQueue() {
    ...
    if (this.annotation_keypoint_queue.length > 0) {
      ...
      this.handleKeypointVisibilityChange(
  ...
  handleKeypointVisibilityChange(annotationIndex, keypointIndex, visibility) {
    ...
    let prev_visibility = this.state.annotations[annotationIndex]["keypoints"][
      keypointIndex * 3 + 2
    ];
    ...
    if (visibility == 0) {
      ...
      if (this.state.annotating) {
        ...
        if (prev_visibility > 0) {
          ...
          let keypoint_layer = this.annotation_layers[annotationIndex][
            "keypoints"
          ][keypointIndex];
        ...
        this.setState(function(prevState, props) {
          prevState.annotations[annotationIndex]["keypoints"][
            keypointIndex * 3 + 2
          ] = 0;
          return {
      ...
      if (this.state.annotating) {
        ...
        var keypoint_layer = this.annotation_layers[annotationIndex][
          "keypoints"
        ][keypointIndex];
        if (keypoint_layer != null) {
          ...
          let keypoint_color = category["keypoints_style"][keypointIndex];
          if (visibility == 1) {
            keypoint_layer.getElement().style.background = this.createKeypointStripedBackgroundStyle(
              keypoint_color
            );
          } else {
            keypoint_layer.getElement().style.background = this.createKeypointSolidBackgroundStyle(
              keypoint_color
            );
          }
    ...
    if (this.props.categories.length == 1) {
      ...
      ReactDOM.render(
        <CategorySelectionModal
          ref={el => {
            this.categorySelectionModalEl = el;
          }}
          categories={this.props.categories}
          cancelled={this.categorySelectionCancelled}
      ...
      $("#categorySelectionModal").on("shown.bs.modal", () => {
        this.categorySelectionModalEl.shown();
      });
  ...
  handleAnnotationDelete(annotation_id) {
    ...
    if (
      annotation_layer.keypoints != "undefined" &&
      annotation_layer.keypoints != null
    ) {
  ...
  getAnnotations() {
    ...
    for (var i = 0; i < annotations.length; i++) {
      ...
      if (
        annotation_layer["bbox"] != "undefined" &&
        annotation_layer["bbox"] != null
      ) {
        let layer = annotation_layer["bbox"];
  ...
  render() {
    ...
    if (this.state.annotating) {
      if (this.annotating_keypoint) {
        let category = this.categoryMap[
          this.state.annotations[this.current_annotationIndex].category_id
        ];
        let keypoint_name = category.keypoints[this.current_keypointIndex];
    ...
    for (var i = 0; i < this.state.annotations.length; i++) {
      ...
      if (this.annotationFeatures != null) {
        // It could be the case that we haven't rendered the map yet.
        hidden = !this.annotationFeatures.hasLayer(
          this.annotation_layers[i]["bbox"]
        );
      }
    ...
    return (
      <div>
              ...
              <div className="p-2">
                <div className="btn-group" role="group">
                  <button
                    ...
                    onClick={this.hideAllAnnotations}
                  >
                    Hide All
                  </button>
                  <button
                    type="button"
                    className="btn btn-outline-secondary"
                    onClick={this.showAllAnnotations}
                  >
                    Show All
                  </button>
                </div>
        ...
        <div className="row">
          <div className="col" />
          <div className="col-10">{instructionsEl}</div>
          <div className="col" />
        </div>
      </div>
    );
